...
def create_settings(settings_file_path):
    ...
    default_settings = app_settings.get_default_app_settings()
    with open(settings_file_path, 'w+') as file:
        default_settings.save_to_file(file)
...
def get_decoded_data(str_data):
    log.info('utils.get_decoded_data')
...
def get_encoded_data(binary_data):
    log.info('utils.get_encoded_data')
...
def get_clipboard_data_on_linux():
    ...
    log.info('utils.get_clipboard_data_on_linux')
    ...
    def _get_data(arg):
        ...
        try:
            ...
            log.exception('utils.get_clipboard_data_on_linux - '
    ...
    if 'text/plain' in list_types:
        ...
        log.info(f'utils.get_clipboard_data_on_linux -- ЭТО ТЕКСТ {encoding}')
        if encoding == 'utf-8':
    ...
    if 'image/png' in list_types:
        log.info('utils.get_clipboard_data_on_linux -- ЭТО ИЗОБРАЖЕНИЕ')
...
def get_clipboard_data_on_windows():
    ...
    log.info('utils.get_clipboard_data_on_windows')
    win32clipboard.OpenClipboard()

    if win32clipboard.IsClipboardFormatAvailable(win32clipboard.CF_UNICODETEXT):
        log.info('utils.get_clipboard_data_on_windows -- ЭТО ТЕКСТ')
    ...
    if win32clipboard.IsClipboardFormatAvailable(win32clipboard.CF_DIB):
        log.info('utils.get_clipboard_data_on_windows -- ЭТО ИЗОБРАЖЕНИЕ')
