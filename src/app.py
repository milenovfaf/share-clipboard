...
class App:
    ...
    def start_app(self):
        ...
        self.settings = utils.load_settings(self.settings_file_path)
        if self.settings is None:
            self.settings = utils.create_settings(self.settings_file_path)
        assert isinstance(self.settings, app_settings.AppSettings)
        log.debug(f'App.start_app - settings: {self.settings.to_dict()}')

        with utils.error_interceptor(
    ...
    def apply_received_data(self, type_data, received_data, show_window=None):
        ...
        if type_data == 'text/plain':
            ...
            with utils.identify_os(
                    utils.paste_data_in_clipboard_on_linux,
                    utils.paste_data_in_clipboard_on_windows
        ...
        if type_data == 'image/png':
            # ---------------------------------------------- #
            received_binary_data = utils.get_decoded_data(received_data)
    ...
    def callback_show_image(self):
        ...
        if self.data_for_create_file:
            ...
            absolute_file_path = utils.create_image_file(
                data,
                'png',
                directory=None
            )
            try:
                utils.open_file_with_os(absolute_file_path)
            except:
                pass
            return
